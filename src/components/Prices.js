      ...
      <p className='text-lg text-blue'>Choose your dog category</p>
        ...
        <div className='grid grid-cols-4 gap-4 lg:gap-[30px]'>
          {bundleData.map((item,idx)=>{
            ...
            return(
              <div  
              onClick={()=>  {setIndex(idx); getBundle(name);}}className='cursor-pointer text-center' key={idx}>
                {}
                <div className='mb-2 lg:mb-8 hover:scale-105 transition-all duration-300'>
                  ...
                  <div className={`${index=== idx? 'border-orange transition-all after:lg:w-5 after:lg:h-5 after:bg-triangle after:lg:bg-no-repeat after:lg:block after:lg:left-[130px] after:lg:absolute after:lg:-bottom-6 relative' :'border-b-4 border-transparent' } pb-4 mb-12`}>
                    <div className='hidden lg:block capitalize'>{dogCategory}
                      </div>
                    </div>
              </div>
            );
